...
/****** Object:  View [dbo].[View_CMP_periodo_puro]    Script Date: 09/10/2015 04:43:50 p. m. ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[View_CMP_periodo_puro]
AS
SELECT        CAST(YEAR(dbo.Lote_CMP.fecha_contable) AS VARCHAR) + RIGHT('00' + CAST(MONTH(dbo.Lote_CMP.fecha_contable) AS VARCHAR), 2) AS Periodo, dbo.Lote_CMP.fecha_contable AS FechaContable, 
                         ...
                         dbo.Lote_CMP.cotizacion_divisa AS CotizacionDivisa, dbo.Lote_CMP.precio_uni AS PrecioUnitario, dbo.Lote_CMP.importe, dbo.Empresas.id AS EmpresaId
FROM            dbo.Lote_CMP INNER JOIN
                         ...
                         dbo.CMP_articulo ON dbo.CMP_articulo.id = dbo.CMP_proveedor_empresa_articulo.articulo_id LEFT OUTER JOIN
                         dbo.CMP_factores_distribucion ON dbo.CMP_factores_distribucion.id = dbo.CMP_proveedor_empresa_articulo.factor_distribucion_id LEFT OUTER JOIN
                         dbo.CMP_tipo_articulo ON dbo.CMP_articulo.tipo_articulo_id = dbo.CMP_tipo_articulo.id
WHERE        (ISNULL(dbo.CMP_proveedor_empresa_articulo.factor_distribucion_id, 1) = 1) AND (dbo.Lote_CMP.destino IS NULL)
